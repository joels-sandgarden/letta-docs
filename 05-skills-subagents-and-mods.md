...
Skills are the lightest surface, and they answer a simple question: what procedure should the agent remember at prompt time? `discoverSkills()` walks four scopes in order of precedence — project, agent, global, bundled — and it resolves duplicate skill IDs by keeping the highest-priority source. Each skill stays a recursive `SKILL.MD` file, which keeps the reuse unit small and legible instead of turning the system into a general knowledge store.
