...
Letta v2 turns the `letta-code` harness into the visible product across the `letta` CLI, the desktop app, `chat.letta.com`, messaging channels, and the Agent SDK. This page exists as the map for the current system: it names the moving parts, shows where the harness ends and the backend begins, and keeps the MemGPT-era server in its historical place.

The harness stays a client. It can talk to three backend implementations: Letta Cloud / Constellation, a self-hosted App Server, and the experimental in-process local engine under `src/backend/local/`. The same turn model reaches all of them, but each backend owns a different slice of the runtime contract.
...
The memory model is described in [Memory blocks and the memory filesystem](./03-memory-blocks-and-the-memory-filesystem.md).
...
The broader extension picture lives in [Dreaming and reflection](./04-dreaming-and-reflection.md) and [Skills, subagents, and mods](./05-skills-subagents-and-mods.md). Tool approval and sandbox rules live in [Tools, permissions, and sandboxing](./06-tools-permissions-and-sandboxing.md). Channel routing lives in [Channels](./07-channels.md).
...
As of mid 2026, the harness releases near daily, the local backend remains experimental, and the newest messaging channels are still young. This page captures the current map, not a promise that every edge will stay fixed.
